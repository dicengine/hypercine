...
bool values_pass(cv::Mat & mat, const int frame, const size_t offset_x=0, const size_t offset_y=0){
  ...
  for(int y=0;y<mat.rows;++y){
    for(int x=0;x<mat.cols;++x){
      diff += mat.at<storage_t>(y,x) - function_value(frame,x+offset_x,y+offset_y);
    }
  }
  return diff==0;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    int frame = 5;
    img = cv::Mat(hc1.height(),hc1.width(),hc1.opencv_data_type(),hc1.data(frame));
    ...
    size_t y_count = 200;
    frame = 12;
    img = cv::Mat(y_count,x_count,hc2.opencv_data_type(),hc2.data(frame,x_begin,x_count,y_begin,y_count));
...
//    cv::imwrite("test.tif",img);
    ...
    DEBUG_MSG("    (read buffer should be called)");
    frame = 11;
    img = cv::Mat(y_count,x_count,hc1.opencv_data_type(),hc1.data(frame,x_begin,x_count,y_begin,y_count));
    ASSERT_EXPR(values_pass(img,frame,x_begin,y_begin),error_count);
    ASSERT_EXPR(hc1.hyperframe()->num_frames()==1,error_count);
    ASSERT_EXPR((*hc1.hyperframe()->get_frame_ids().begin()==frame),error_count);
    ASSERT_EXPR(hc1.hyperframe()->num_windows()==1,error_count);
    ...
    frame = 7;
    size_t window_id = 3;
    img = cv::Mat(y_count+3,x_count+100,hc1.opencv_data_type(),hc1.data(frame,window_id));
    ...
    DEBUG_MSG("    (no calls to read buffer should be made)");
    frame = 3;
    img = cv::Mat(y_count+3,x_count+100,hc1.opencv_data_type(),hc1.data(frame,x_begin+12,x_count+100,y_begin+19,y_count+3));
    ASSERT_EXPR(values_pass(img,frame,x_begin+12,y_begin+19),error_count);
    ASSERT_EXPR(hc1.hyperframe()->num_frames()==11,error_count);
    ASSERT_EXPR(hc1.hyperframe()->num_windows()==4,error_count);
    ...
    DEBUG_MSG("    (read buffer should be called, but the window dims should remain the same)");
    frame = 0;
    img = cv::Mat(y_count,x_count+100,hc1.opencv_data_type(),hc1.data(frame,2));
    ...
    try{
      ...
      storage_t * ptr = hc3.data(frame,2);
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc1.data(frame,17);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc1.data(frame+2000,17);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc3.data(frame+2000);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc1.data(frame+2000);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc3.data(frame,x_begin,x_count,y_begin,y_count+2000);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
    ...
    try{
      std::cout << "expecting error" << std::endl;
      storage_t * ptr = hc1.data(frame,x_begin,x_count,y_begin,y_count+2000);
      // exception should throw before this step
      ASSERT_EXPR(false,error_count);
    } catch(std::exception &e){
    }
