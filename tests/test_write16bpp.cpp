...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    base_img.convertTo(base_img, CV_16UC1);

    // write one image to clear out the output cine file if it exists
    const std::string file_name = "test_write16bpp.cine";
    HyperCine::write_frame(file_name.c_str(),base_img.cols,base_img.rows,(uint16_t*)base_img.data,true);
...
//      cv::imwrite(img_text.str(),img);
      HyperCine::write_frame(file_name.c_str(),img.cols,img.rows,(uint16_t*)img.data);
    }
    // test file size
    ...
    const int per_image_storage = base_img.rows * base_img.cols * 2; // 16 bit
